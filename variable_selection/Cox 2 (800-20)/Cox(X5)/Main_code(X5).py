...
def single_simulation(b, n, Dnn_layer1, Dnn_node1, Dnn_lr1, Dnn_layer2, Dnn_node2, Dnn_lr2, Dnn_epoch, patiences, tau, corr):
    ...
    if 'X' in Data_all_ic:
        Data_all_ic['X'] = np.delete(Data_all_ic['X'], 4, axis=1)
...
# 3) 索引（从 1 开始，便于人读/与统计软件一致）
# idx1 = idx0 + 1

# 保存
# np.save('mask_leq_1p96.npy', mask)           # 保存为 numpy 二进制
np.savetxt('file5.txt', idx0, fmt='%d')  # 0-based 索引
# np.savetxt('idx1_leq_1p96.txt', idx1, fmt='%d')  # 1-based 索引
