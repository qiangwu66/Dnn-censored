...
def single_simulation(b, n, Dnn_layer, Dnn_node, Dnn_lr, Dnn_epoch, tau, patiences):
    ...
    g_t_X_true_Cox = np.log(np.sqrt(T_O_train + 0.001) / 6) + f_X_train # n
...
size_DNN_path = os.path.join(output_folder, f"size_DNN-{n}-{Dnn_layer}-{Dnn_node}-{Dnn_lr}-new.csv")
...
fig1.suptitle(r"QQ-Plots under AFT model", fontsize=10)  
...
res1 = stats.probplot(T_Sigma_1_AFT_B, dist="norm", plot=ax1_1)
res2 = stats.probplot(T_Sigma_2_AFT_B, dist="norm", plot=ax1_2)
res3 = stats.probplot(T_Sigma_3_AFT_B, dist="norm", plot=ax1_3)
res4 = stats.probplot(T_Sigma_4_AFT_B, dist="norm", plot=ax1_4)
...
fig1.savefig("qq_plot_AFT.pdf", dpi=600)
