...
# N1
Data_N1 = generate_AH_1(2000, corr, tau, c2)
De_N1 = Data_N1['De']
f_X_N1 = Data_N1['f_X']
Y_N1 = Data_N1['T_O']

# N2
Data_N2 = generate_AH_2_cross(2000, corr, tau)
De_N2 = Data_N2['De']
f_X_N2 = Data_N2['f_X']
Y_N2 = Data_N2['T_O']
...
# g1,S1
g1_true_N1 = np.log(c2 + f_X_N1) 
g1_true_N2 = np.log(c2 + f_X_N2)

S1_true_N1 = np.exp(-Y_N1 * (c2 + f_X_N1)) 
S1_true_N2 = np.exp(-Y_N2 * (c2 + f_X_N2))
...
# g2,S2
g2_true_N1 = np.log((Y_N1 + 0.001) / 2 + f_X_N1) 
g2_true_N2 = np.log((Y_N2 + 0.001) / 2 + f_X_N2)

S2_true_N1 = np.exp(-(((Y_N1 + 0.001) ** 2 - 1e-6) / 4 + Y_N1 * f_X_N1)) 
S2_true_N2 = np.exp(-(((Y_N2 + 0.001) ** 2 - 1e-6) / 4 + Y_N2 * f_X_N2))

f2_true_N1 = np.exp(g2_true_N1) * S2_true_N1 
f2_true_N2 = np.exp(g2_true_N2) * S2_true_N2


Data_gene = generate_AH_1(5000, corr, tau, c2)
X_gene = Data_gene['X'] 
f_X_gene = Data_gene['f_X']
C_gene = Data_gene['De']


S1_t_X_gene_N1 = np.exp(- Y_N1[:,np.newaxis] * (c2 + f_X_gene))

S1_t_X_gene_N2 = np.exp(- Y_N2[:,np.newaxis] * (c2 + f_X_gene))
...
W2_1_N2 = E_S_N2 * E_I_C_T_N2
W2_2_N2 = E_S_N2 * E_I_C_T_N2 * S1_true_N2
W2_3_N2 = E_S_N2 * E_I_C_T_N2 * np.sqrt(S1_true_N2 * (1-S1_true_N2))
W2_4_N2 = E_S_N2 * E_I_C_T_N2 * S1_true_N2 * (1-S1_true_N2)


Eta_1 = (np.mean(De_N1 * W_1_N1 * (g1_true_N1 - g2_true_N1)) + np.mean(De_N2 * W2_1_N2 * (g1_true_N2 - g2_true_N2))) / 2
Eta_2 = (np.mean(De_N1 * W_2_N1 * (g1_true_N1 - g2_true_N1)) + np.mean(De_N2 * W2_2_N2 * (g1_true_N2 - g2_true_N2))) / 2
Eta_3 = (np.mean(De_N1 * W_3_N1 * (g1_true_N1 - g2_true_N1)) + np.mean(De_N2 * W2_3_N2 * (g1_true_N2 - g2_true_N2))) / 2
Eta_4 = (np.mean(De_N1 * W_4_N1 * (g1_true_N1 - g2_true_N1)) + np.mean(De_N2 * W2_4_N2 * (g1_true_N2 - g2_true_N2))) / 2
...
g1_T_nodes_X_N1 = np.tile(np.log(c2 + f_X_N1), (5000, 1)) 
g1_T_nodes_X_N2 = np.tile(np.log(c2 + f_X_N1), (5000, 1)) 
S1_T_nodes_X_gene = np.exp(-T_nodes[:,np.newaxis] * (c2 + f_X_gene))
E_S1_T_nodes = np.mean(S1_T_nodes_X_gene, axis=1) 
S1_T_nodes_X_N1 = np.exp(-T_nodes[:,np.newaxis] * (c2 + f_X_N1)) 
S1_T_nodes_X_N2 = np.exp(-T_nodes[:,np.newaxis] * (c2 + f_X_N2)) 
...
g2_T_nodes_X_N1 = np.log((T_nodes.reshape(-1, 1) + 0.001) / 2 + f_X_N1.reshape(1, -1)) + c2 
g2_T_nodes_X_N2 = np.log((T_nodes.reshape(-1, 1) + 0.001) / 2 + f_X_N2.reshape(1, -1)) + c2 
S2_T_nodes_X_gene = np.exp(-(((T_nodes[:,np.newaxis] + 0.001) ** 2 - 1e-6) / 4 + T_nodes[:,np.newaxis] * f_X_gene))
E_S2_T_nodes = np.mean(S2_T_nodes_X_gene, axis=1) 
S2_T_nodes_X_N1 = np.exp(-(((T_nodes[:,np.newaxis] + 0.001) ** 2 - 1e-6) / 4 + T_nodes[:,np.newaxis] * f_X_N1)) 
S2_T_nodes_X_N2 = np.exp(-(((T_nodes[:,np.newaxis] + 0.001) ** 2 - 1e-6) / 4 + T_nodes[:,np.newaxis] * f_X_N2)) 
...
W2_T_nodes_1_N2 =  np.tile((E_S1_T_nodes * E_I_C_T_nodes)[:, np.newaxis], (1, 2000)) 
W2_T_nodes_2_N2 =  np.tile((E_S1_T_nodes * E_I_C_T_nodes)[:, np.newaxis], (1, 2000)) * S2_T_nodes_X_N2 
W2_T_nodes_3_N2 =  np.tile((E_S1_T_nodes * E_I_C_T_nodes)[:, np.newaxis], (1, 2000)) * np.sqrt(S2_T_nodes_X_N2 * (1 - S2_T_nodes_X_N2)) 
W2_T_nodes_4_N2 =  np.tile((E_S1_T_nodes * E_I_C_T_nodes)[:, np.newaxis], (1, 2000)) * S2_T_nodes_X_N2 * (1 - S2_T_nodes_X_N2) 
...
Psi_1_N2 = De_N2 * W2_1_N2 - tau * np.mean(I_T_T_nodes_N2.T * np.exp(g2_T_nodes_X_N2) * W2_T_nodes_1_N2, axis=0)
Psi_2_N2 = De_N2 * W2_2_N2 - tau * np.mean(I_T_T_nodes_N2.T * np.exp(g2_T_nodes_X_N2) * W2_T_nodes_2_N2, axis=0)
Psi_3_N2 = De_N2 * W2_3_N2 - tau * np.mean(I_T_T_nodes_N2.T * np.exp(g2_T_nodes_X_N2) * W2_T_nodes_3_N2, axis=0)
Psi_4_N2 = De_N2 * W2_4_N2 - tau * np.mean(I_T_T_nodes_N2.T * np.exp(g2_T_nodes_X_N2) * W2_T_nodes_4_N2, axis=0)
...
Theta_1_N2 = De_N2 * (1 + f1_true_N2 / f2_true_N2) * W2_1_N2 - tau * np.mean(I_T_T_nodes_N2.T * np.exp(g2_T_nodes_X_N2) * (1 + f1_T_nodes_X_N2 / f2_T_nodes_X_N2) * W2_T_nodes_1_N2, axis=0)
Theta_2_N2 = De_N2 * (1 + f1_true_N2 / f2_true_N2) * W2_2_N2 - tau * np.mean(I_T_T_nodes_N2.T * np.exp(g2_T_nodes_X_N2) * (1 + f1_T_nodes_X_N2 / f2_T_nodes_X_N2) * W2_T_nodes_2_N2, axis=0)
Theta_3_N2 = De_N2 * (1 + f1_true_N2 / f2_true_N2) * W2_3_N2 - tau * np.mean(I_T_T_nodes_N2.T * np.exp(g2_T_nodes_X_N2) * (1 + f1_T_nodes_X_N2 / f2_T_nodes_X_N2) * W2_T_nodes_3_N2, axis=0)
Theta_4_N2 = De_N2 * (1 + f1_true_N2 / f2_true_N2) * W2_4_N2 - tau * np.mean(I_T_T_nodes_N2.T * np.exp(g2_T_nodes_X_N2) * (1 + f1_T_nodes_X_N2 / f2_T_nodes_X_N2) * W2_T_nodes_4_N2, axis=0)
...
Omega_1_N2 = Theta_1_N2 + De_N2 * W2_1_N2 * (g1_true_N2 - g2_true_N2)
Omega_2_N2 = Theta_2_N2 + De_N2 * W2_2_N2 * (g1_true_N2 - g2_true_N2)
Omega_3_N2 = Theta_3_N2 + De_N2 * W2_3_N2 * (g1_true_N2 - g2_true_N2)
Omega_4_N2 = Theta_4_N2 + De_N2 * W2_4_N2 * (g1_true_N2 - g2_true_N2)
...
size_DNN_path_new = f"result_True_power_AH_cross.csv"  
