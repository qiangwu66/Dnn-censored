...
import torch
from data_generator import generate_Cox_1, generate_Cox_2_cross
import pandas as pd
...
# N2
Data_N2 = generate_Cox_2_cross(2000, corr, tau)
De_N2 = Data_N2['De']
f_X_N2 = Data_N2['f_X']
Y_N2 = Data_N2['T_O']
...
# g1,S1
g1_true_N1 = np.log((Y_N1 + 0.001) / 3) + f_X_N1
g1_true_N2 = np.log((Y_N2 + 0.001) / 3) + f_X_N2 
...
# g2,S2
g2_true_N1 = np.log((Y_N1 + 0.001) ** 2 / 3) + f_X_N1
g2_true_N2 = np.log((Y_N2 + 0.001) ** 2 / 3) + f_X_N2 

S2_true_N1 = np.exp(- ((Y_N1 + 0.001) ** 3 / 9 - 0.001 ** 3 / 9) * np.exp(f_X_N1))
S2_true_N2 = np.exp(- ((Y_N2 + 0.001) ** 3 / 9 - 0.001 ** 3 / 9) * np.exp(f_X_N2)) 
...
S1_t_X_gene_N1 = np.exp(- ((Y_N1[:,np.newaxis] + 0.001) ** 2 / 6 - 0.001 ** 2 / 6) * np.exp(f_X_gene))
S2_t_X_gene_N2 = np.exp(- ((Y_N2[:,np.newaxis] + 0.001) ** 3 / 9 - 0.001 ** 3 / 9) * np.exp(f_X_gene))
...
g2_T_nodes_X_N1 = np.log((T_nodes.reshape(-1, 1) + 0.001) ** 2 / 3) + f_X_N1.reshape(1, -1) 
g2_T_nodes_X_N2 = np.log((T_nodes.reshape(-1, 1) + 0.001) ** 2 / 3) + f_X_N2.reshape(1, -1) 
S2_T_nodes_X_gene = np.exp(- ((T_nodes[:,np.newaxis] + 0.001) ** 3 / 9 - 0.001 ** 3 / 9) * np.exp(f_X_gene)) 
E_S2_T_nodes = np.mean(S2_T_nodes_X_gene, axis=1) 
S2_T_nodes_X_N1 = np.exp(- ((T_nodes[:,np.newaxis] + 0.001) ** 3 / 9 - 0.001 ** 3 / 9) * np.exp(f_X_N1)) 
S2_T_nodes_X_N2 = np.exp(- ((T_nodes[:,np.newaxis] + 0.001) ** 3 / 9 - 0.001 ** 3 / 9) * np.exp(f_X_N2)) 
...
W_T_nodes_1_N2 =  np.tile((E_S2_T_nodes * E_I_C_T_nodes)[:, np.newaxis], (1, 2000)) 
W_T_nodes_2_N2 =  np.tile((E_S2_T_nodes * E_I_C_T_nodes)[:, np.newaxis], (1, 2000)) * S2_T_nodes_X_N2 
W_T_nodes_3_N2 =  np.tile((E_S2_T_nodes * E_I_C_T_nodes)[:, np.newaxis], (1, 2000)) * np.sqrt(S2_T_nodes_X_N2 * (1 - S2_T_nodes_X_N2)) 
W_T_nodes_4_N2 =  np.tile((E_S2_T_nodes * E_I_C_T_nodes)[:, np.newaxis], (1, 2000)) * S2_T_nodes_X_N2 * (1 - S2_T_nodes_X_N2) 
...
size_DNN_path_new = f"result_True_power_Cox_cross.csv" 
