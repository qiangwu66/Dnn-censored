...
Data1 = generate_AH_1(n, corr, tau, c2)
X = Data1['X']
f_X = Data1['f_X']
...
for k in range(n):
    S1_t_X = np.exp(- t_fig * (c2 + f_X[k]))
    S2_t_X = np.exp(-(((t_fig + 0.001) ** 2 - 1e-6) / 4 + t_fig * f_X[k]))
    fig1 = plt.figure() 
    fig1.suptitle("AH", fontsize=10) 
    ...
    file_name = os.path.join("Individual_DNN", f'AH_cross_fig_{k}.pdf') 
