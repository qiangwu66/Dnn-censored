...
def single_simulation(b, n, Dnn_layer1, Dnn_node1, Dnn_lr1, Dnn_layer2, Dnn_node2, Dnn_lr2, Dnn_epoch, patiences, tau, corr):
    ...
    set_seed(179 + b)
    # --------------g1(t,x)------------
    Data1_all = generate_AFT_1(n, corr, tau) 
    ...
    # --------------g2(t,x)-------------
    Data2_all = generate_AFT_2_cross(n, corr, tau) 
    ...
    # -----------------------------------------------------------
    U_tau_w1_00.append((abs(Est_dnn_g1['U_tau_w1']) > 1.96))
    U_tau_w2_00.append((abs(Est_dnn_g1['U_tau_w2']) > 1.96))
    U_tau_w3_00.append((abs(Est_dnn_g1['U_tau_w3']) > 1.96))
    U_tau_w4_00.append((abs(Est_dnn_g1['U_tau_w4']) > 1.96))
...
size1_AFT = np.mean(abs(U_tau_w1_B) > 1.96)
size2_AFT = np.mean(abs(U_tau_w2_B) > 1.96)
size3_AFT = np.mean(abs(U_tau_w3_B) > 1.96)
size4_AFT = np.mean(abs(U_tau_w4_B) > 1.96)
...
result_size_DNN = pd.DataFrame(
    np.array([size1_AFT, size2_AFT, size3_AFT, size4_AFT])
    ) 
size_DNN_path = os.path.join(output_folder, f"cross-AFT-size_DNN-{Dnn_layer1}-{Dnn_node1}-{Dnn_lr1}-{Dnn_layer2}-{Dnn_node2}-{Dnn_lr2}.csv")
