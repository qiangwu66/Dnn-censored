...
Data1 = generate_Cox_1(n, corr, tau)
X = Data1['X']
f_X = Data1['f_X']
...
for k in range(n):
    S1_t_X = np.exp(-(((t_fig + 0.001) ** 2 - 1e-6) / 6) * np.exp(f_X[k]))
    S2_t_X = np.exp(- ((t_fig + 0.001) ** 3 / 9 - 0.001 ** 3 / 9) * np.exp(f_X[k]))
    fig1 = plt.figure() 
    fig1.suptitle("Cox", fontsize=10) 
    ...
    file_name = os.path.join("Individual_DNN", f'Cox_cross_fig_{k}.pdf') 
