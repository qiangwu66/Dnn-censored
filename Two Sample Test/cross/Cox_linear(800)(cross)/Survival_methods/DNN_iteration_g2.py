...
def g2_dnn(polled_data, train_data, val_data, tau, n_layer, n_node, n_lr, n_epoch, patiences):
    ...
    with torch.no_grad():
        ...
        S_T_X_n = torch.exp(-int_exp_g_TX) 
        g_T_X_n = model(torch.cat((T_O_all.unsqueeze(1), X_all), dim=1)).squeeze() 
        ...
        sigma2_ws =  torch.tensor([sigma_1, sigma_2, sigma_3, sigma_4])

    
    return {
        'g2_T_X_n': g_T_X_n.detach().numpy(), 
        'sigma2_ws': sigma2_ws.detach().numpy(), 
    }
