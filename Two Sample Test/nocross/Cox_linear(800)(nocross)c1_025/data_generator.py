...
def generate_Cox_1(n, corr, tau):
    ...
    # lambda_1(t|X)= [(t+0.001)/3] * exp(f_X)
    # Lambda_0(t)= [((t+0.001)**2 - 1e-6)/6] * exp(f_X)
    Y = ndm.rand(n)
...
def generate_Cox_2_nocross(n, corr, tau, c1):
    ...
    X1 = (np.clip(X0, -2, 2) + 2) / 4
    f_X = Z1 + Z2 / 2 + X1[:,0] / 3 + X1[:,1] / 4 + X1[:,2] / 5 +  X1[:,3] / 6
    # lambda_1(t|X)= exp(c1) * [(t+0.001)/3] * exp(f_X)
    # Lambda_0(t)= [((t+0.001)**2 - 1e-6)/6] * exp(f_X + c1)
    Y = ndm.rand(n) 
    T = np.sqrt(1e-6 - 6 * np.log(Y) * np.exp(-f_X -c1)) - 1e-3
    # C = uniform_data(n, tau / 2, tau)
    C = np.minimum(ndm.exponential(2, n), tau)
    De = (T <= C)
    T_O = np.minimum(T, C)
    Z1 = Z1.reshape(n, 1)
