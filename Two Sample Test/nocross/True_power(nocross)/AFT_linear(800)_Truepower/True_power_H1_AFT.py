...
import torch
from data_generator import generate_AFT_1, generate_AFT_2_nocross
import pandas as pd
...
# N2
Data_N2 = generate_AFT_2_nocross(2000, corr, tau, c1)
De_N2 = Data_N2['De']
f_X_N2 = Data_N2['f_X']
Y_N2 = Data_N2['T_O']
...
# g2,S2
g2_true_N1 = np.log(2 * Y_N1) - 2 * f_X_N1 + c1 
g2_true_N2 = np.log(2 * Y_N2) - 2 * f_X_N2 + c1 

S2_true_N1 = np.exp(- Y_N1 ** 2 * np.exp(c1 - 2 * f_X_N1)) 
S2_true_N2 = np.exp(- Y_N2 ** 2 * np.exp(c1 - 2 * f_X_N2)) 
...
S2_t_X_gene_N1 = np.exp(- Y_N1[:,np.newaxis] ** 2 * np.exp(c1 - 2 * f_X_gene))

S2_t_X_gene_N2 = np.exp(- Y_N2[:,np.newaxis] ** 2 * np.exp(c1 - 2 * f_X_gene))
...
g2_T_nodes_X_N1 = np.log(2 * T_nodes.reshape(-1, 1)) - 2 * f_X_N1.reshape(1, -1) + c1
g2_T_nodes_X_N2 = np.log(2 * T_nodes.reshape(-1, 1)) - 2 * f_X_N2.reshape(1, -1) + c1
S2_T_nodes_X_gene = np.exp(- T_nodes[:,np.newaxis] ** 2 * np.exp(c1 - 2 * f_X_gene))
E_S2_T_nodes = np.mean(S2_T_nodes_X_gene, axis=1) 
S2_T_nodes_X_N1 = np.exp(- T_nodes[:,np.newaxis] ** 2 * np.exp(c1 - 2 * f_X_N1))
S2_T_nodes_X_N2 = np.exp(- T_nodes[:,np.newaxis] ** 2 * np.exp(c1 - 2 * f_X_N2))
...
size_DNN_path_new = f"result_True_power_AFT_nocross_{c1}.csv"  
