...
import torch
from data_generator import generate_AH_1, generate_AH_2_nocross
import pandas as pd
...
# N1
Data_N1 = generate_AH_1(2000, corr, tau)
De_N1 = Data_N1['De']
f_X_N1 = Data_N1['f_X']
Y_N1 = Data_N1['T_O']

# N2
Data_N2 = generate_AH_2_nocross(2000, corr, tau, c1)
De_N2 = Data_N2['De']
f_X_N2 = Data_N2['f_X']
Y_N2 = Data_N2['T_O']

# g1,S1
g1_true_N1 = np.log((Y_N1 + 0.001) / 20 + f_X_N1) 
g1_true_N2 = np.log((Y_N2 + 0.001) / 20 + f_X_N2) 

S1_true_N1 = np.exp(-(((Y_N1 + 0.001) ** 2 - 1e-6) / 40 + Y_N1 * f_X_N1)) 
S1_true_N2 = np.exp(-(((Y_N2 + 0.001) ** 2 - 1e-6) / 40 + Y_N2 * f_X_N2)) 

f1_true_N1 = np.exp(g1_true_N1) * S1_true_N1 
f1_true_N2 = np.exp(g1_true_N2) * S1_true_N2 

# g2,S2
g2_true_N1 = np.log((Y_N1 + 0.001) / 20 + f_X_N1) + c1 
g2_true_N2 = np.log((Y_N2 + 0.001) / 20 + f_X_N2) + c1 

S2_true_N1 = np.exp(-np.exp(c1) * (((Y_N1 + 0.001) ** 2 - 1e-6) / 40 + Y_N1 * f_X_N1)) 
S2_true_N2 = np.exp(-np.exp(c1) * (((Y_N2 + 0.001) ** 2 - 1e-6) / 40 + Y_N2 * f_X_N2)) 

f2_true_N1 = np.exp(g2_true_N1) * S2_true_N1 
f2_true_N2 = np.exp(g2_true_N2) * S2_true_N2 

Data_gene = generate_AH_1(5000, corr, tau)
X_gene = Data_gene['X'] 
f_X_gene = Data_gene['f_X']
C_gene = Data_gene['De']

S2_t_X_gene_N1 = np.exp(-np.exp(c1) * (((Y_N1[:,np.newaxis] + 0.001) ** 2 - 1e-6) / 40 + Y_N1[:,np.newaxis] * f_X_gene))

S2_t_X_gene_N2 = np.exp(-np.exp(c1) * (((Y_N2[:,np.newaxis] + 0.001) ** 2 - 1e-6) / 40 + Y_N2[:,np.newaxis] * f_X_gene))
...
Lambda_T_nodes = ((T_nodes + 0.001) ** 2 - 1e-6) / 40 
E_I_C_T_nodes =  np.mean(Indicator_matrix(C_gene, T_nodes), axis=0) 


I_T_T_nodes_N1 = Indicator_matrix(Y_N1, T_nodes)
g1_T_nodes_X_N1 = np.log((T_nodes.reshape(-1, 1) + 0.001) / 20 + f_X_N1.reshape(1, -1))
g1_T_nodes_X_N2 = np.log((T_nodes.reshape(-1, 1) + 0.001) / 20 + f_X_N2.reshape(1, -1))
S1_T_nodes_X_gene = np.exp(-Lambda_T_nodes[:,np.newaxis] - T_nodes[:,np.newaxis] * f_X_gene) 
E_S1_T_nodes = np.mean(S1_T_nodes_X_gene, axis=1)  
S1_T_nodes_X_N1 = np.exp(-Lambda_T_nodes[:,np.newaxis] - T_nodes[:,np.newaxis] * f_X_N1)
S1_T_nodes_X_N2 = np.exp(-Lambda_T_nodes[:,np.newaxis] - T_nodes[:,np.newaxis] * f_X_N2)
...
g2_T_nodes_X_N1 = np.log((T_nodes.reshape(-1, 1) + 0.001) / 20 + f_X_N1.reshape(1, -1)) + c1
g2_T_nodes_X_N2 = np.log((T_nodes.reshape(-1, 1) + 0.001) / 20 + f_X_N2.reshape(1, -1)) + c1
S2_T_nodes_X_gene = np.exp(-np.exp(c1) * (Lambda_T_nodes[:,np.newaxis] + T_nodes[:,np.newaxis] * f_X_gene)) 
E_S2_T_nodes = np.mean(S2_T_nodes_X_gene, axis=1)  
S2_T_nodes_X_N1 = np.exp(-np.exp(c1) * (Lambda_T_nodes[:,np.newaxis] + T_nodes[:,np.newaxis] * f_X_N1))
S2_T_nodes_X_N2 = np.exp(-np.exp(c1) * (Lambda_T_nodes[:,np.newaxis] + T_nodes[:,np.newaxis] * f_X_N2))
...
W_T_nodes_4_N1 =  np.tile((E_S1_T_nodes * E_I_C_T_nodes)[:, np.newaxis], (1, 2000)) * S2_T_nodes_X_N1 * (1 - S1_T_nodes_X_N1)
...
size_DNN_path_new = f"result_True_power_AH_nocross_{c1}.csv" 
