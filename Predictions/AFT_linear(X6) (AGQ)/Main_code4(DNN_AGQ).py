...
s = 500
Data_test = generate_case_4(s, corr, tau)
X_test = Data_test['X'] 
...
St_true_X_DMS = 1 - norm.cdf((np.log(T_O_test) - f_X_test) / 1) 
...
St_true_X_fig = 1 - norm.cdf((np.repeat(np.log(t_fig)[:,np.newaxis],X_test.shape[0],axis=1) - np.repeat(f_X_test[np.newaxis,:],len(t_fig),axis=0)) / 1)
...
def single_simulation(b, n, Dnn_layer, Dnn_node, Dnn_epoch, Dnn_lr, t_nodes, t_fig, tau, De_test, St_true_X_DMS, T_O_test, s_k):
    ...
    set_seed(20 + b)
    # --------------------------
    Data_all = generate_case_4(n, corr, tau) 
...
DMS_path = os.path.join(output_folder, "DMS_DNN_AGQ.csv")
...
ibs_path = os.path.join(output_folder, "IBS_DNN_AGQ.csv")
...
run_time_path = os.path.join(output_folder, "run_time_DNN_AGQ.csv")
...
for k in range(10):
    fig1 = plt.figure()
    fig1.suptitle("(d) AFT I", fontsize=10) 
    # -----n=400, X-----
    ...
    ax1_1.plot(t_fig, St_DNN_X_400[k], color='green', label='Proposed (AGQ)', linestyle='-.')
    ax1_1.legend(loc='best', fontsize=6)
    # n=800
    ax1_2.plot(t_fig, St_true_X_fig[:, k], color='black', label='True', linestyle='-')
    ax1_2.plot(t_fig, St_DNN_X_800[k], color='green', label='Proposed (AGQ)', linestyle='-.')
    ax1_2.legend(loc='best', fontsize=6)
    file_name = os.path.join(subfolder, f'DNN_Case4_fig_{k}_AGQ.jpeg')  
