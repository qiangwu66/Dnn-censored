...
#%% ========================
def single_simulation(b, n, t_nodes, t_fig, tau, De_test, St_true_X_DMS, T_O_test, s_k, m_PH, nodevec_PH):
    print(f'-------------n={n}, b={b}--------------')
    set_seed(20 + b)
    # -----------------------
...
for ii in range(len(Set_n)):
    ...
    if ii == 0: 
        results_400 = Parallel(n_jobs=n_jobs)(
            delayed(single_simulation)(b, n, t_nodes, t_fig, tau, De_test, St_true_X_DMS, T_O_test, s_k, m_PH, nodevec_PH) 
            for b in range(B)
        )
    else: 
...
for k in range(10):
    ...
    file_name = os.path.join(subfolder, f'CoxPH_Case4_fig_{k}.jpeg')  
