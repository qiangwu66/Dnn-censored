...
St_true_X_DMS = np.exp(-((T_O_test + 0.001) ** (3 / 2) / 21) * np.exp(f_X_test)) 
...
Lambda_t_true = (t_fig + 0.001) ** (3/2) / 21
...
for k in range(10):
    fig1 = plt.figure() 
    fig1.suptitle("(b) Cox II", fontsize=10)
    # -----n=400, X-----
    ...
    file_name = os.path.join(subfolder, f'DNN_Case2_fig_{k}_AGQ.jpeg') 
