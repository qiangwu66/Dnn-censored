...
G_T_i = np.maximum(kmf.predict(T_O_test).values, np.min(kmf.predict(T_O_test).values[kmf.predict(T_O_test).values > 0])) # s = 200
...
def single_simulation(b, n, t_nodes, t_fig, tau, De_test, St_true_X_DMS, X_test, T_O_test, s_k, m_AH, nodevec_AH):
    ...
    S_t_X_CoxCC_DMS = surv.apply(lambda col: np.interp(T_O_test, surv.index, col), axis=0).to_numpy() # 200*200
    ...
    S_t_X_DeepSurv_DMS = surv_DeepSurv.apply(lambda col: np.interp(T_O_test, surv_DeepSurv.index, col), axis=0).to_numpy() # 200*200
...
for k in range(10):
    ...
    file_name = os.path.join(subfolder, f'Nine_Case1_fig_{k}.jpeg')  
