...
def g_dnn(train_data, val_data, test_data, t_nodes, t_fig, s_k, n_layer, n_node, n_lr, n_epoch, patiences):
    ...
    with torch.no_grad():
        ...
        for k in range(len(t_fig)):
            ...
            int_exp_g_TX = vectorized_gaussian_quadrature_integral(batch_func_test, torch.zeros_like(t_fig_k_repeat), t_fig_k_repeat, n_points=100)
            S_T_X_fig[k] = torch.exp(-int_exp_g_TX)
 
        
        # DMS
        int_exp_g_TX = vectorized_gaussian_quadrature_integral(batch_func_test, torch.zeros_like(T_O_test), T_O_test, n_points=100)
    ...
    return {
        'S_T_X_fig': S_T_X_fig.T.detach().numpy(),  
        'S_T_X_ibs': S_T_X_ibs.T.detach().numpy(),  
        'S_T_X_DMS': S_T_X_DMS.detach().numpy()   
    }
