...
Lambda_t_true = (t_range + 0.001) ** 2 / 2 - 0.001 ** 2 / 2 
S_t_X_true = np.exp(-np.repeat(Lambda_t_true[:,np.newaxis],X_test.shape[0],axis=1) - np.repeat(t_range[:,np.newaxis],X_test.shape[0],axis=1) * np.repeat(f_X_test[np.newaxis,:],len(t_range),axis=0))



np.save(os.path.join(results_folder, 't_range_AH.npy'), t_range)
np.save(os.path.join(results_folder, 'S_t_X_true_AH.npy'), S_t_X_true)
...
S_t_X_true = data['S_t_X_true_AH.npy'] 
Se_b = data['Se_b.npy']
t_range = data['t_range_AH.npy'] 
...
# =================tables=======================
output_folder = "Individual_AH"
os.makedirs(output_folder, exist_ok=True)


result_t_range = pd.DataFrame(t_range)  

t_range_path = os.path.join(output_folder, "t_range_AH.csv")
...
S_t_X_true_path = os.path.join(output_folder, "S_t_X_true_AH.csv")
result_S_t_X_true.to_csv(S_t_X_true_path, index=False, header=False) 


result_Bias = pd.DataFrame(Bias)  

Bias_path = os.path.join(output_folder, f"Bias_AH-{Dnn_layer}-{Dnn_node}-{Dnn_lr}.csv")
...
SD_path = os.path.join(output_folder, f"SD_AH-{Dnn_layer}-{Dnn_node}-{Dnn_lr}.csv")
...
CP_path = os.path.join(output_folder, f"CP_AH-{Dnn_layer}-{Dnn_node}-{Dnn_lr}.csv")
...
CP_se_path = os.path.join(output_folder, f"CP_se_AH-{Dnn_layer}-{Dnn_node}-{Dnn_lr}.csv")
result_CP_se.to_csv(CP_se_path, index=False, header=False) 


result_se = pd.DataFrame(Se_B)  

Se_B_path = os.path.join(output_folder, f"Se_B_AH-{Dnn_layer}-{Dnn_node}-{Dnn_lr}.csv")
