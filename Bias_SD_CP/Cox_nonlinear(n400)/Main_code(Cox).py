...
s = 10
Data_test = generate_case_2(s, corr, tau)
X_test = Data_test['X']
...
Lambda_t_true = (t_range + 0.001) ** (3/2) / 21 - 0.001 ** (3/2) / 21
S_t_X_true = np.exp(-np.repeat(Lambda_t_true[:,np.newaxis],X_test.shape[0],axis=1) * np.exp(np.repeat(f_X_test[np.newaxis,:],len(t_range),axis=0))) 

np.save(os.path.join(results_folder, 't_range_Cox.npy'), t_range)
np.save(os.path.join(results_folder, 'S_t_X_true_Cox.npy'), S_t_X_true)
...
def single_simulation(b, n, Dnn_layer, Dnn_node, Dnn_lr, Dnn_epoch, t_range, Data_test, tau, patiences, num_bs):
    ...
    Data_all = generate_case_2(n, corr, tau) 
...
S_t_X_true = data['S_t_X_true_Cox.npy'] 
Se_b = data['Se_b.npy'] 
t_range = data['t_range_Cox.npy'] 
...
# =================tables=======================
output_folder = "Individual_Cox"
os.makedirs(output_folder, exist_ok=True)


result_t_range = pd.DataFrame(t_range)  

t_range_path = os.path.join(output_folder, "t_range_Cox.csv")
...
S_t_X_true_path = os.path.join(output_folder, "S_t_X_true_Cox.csv")
result_S_t_X_true.to_csv(S_t_X_true_path, index=False, header=False) 


result_Bias = pd.DataFrame(Bias)  

Bias_path = os.path.join(output_folder, f"Bias_Cox-{Dnn_layer}-{Dnn_node}-{Dnn_lr}.csv")
...
SD_path = os.path.join(output_folder, f"SD_Cox-{Dnn_layer}-{Dnn_node}-{Dnn_lr}.csv")
...
CP_path = os.path.join(output_folder, f"CP_Cox-{Dnn_layer}-{Dnn_node}-{Dnn_lr}.csv")
...
CP_se_path = os.path.join(output_folder, f"CP_se_Cox-{Dnn_layer}-{Dnn_node}-{Dnn_lr}.csv")
...
Se_B_path = os.path.join(output_folder, f"Se_B_Cox-{Dnn_layer}-{Dnn_node}-{Dnn_lr}.csv")
