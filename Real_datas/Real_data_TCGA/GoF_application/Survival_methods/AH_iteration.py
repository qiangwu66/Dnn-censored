...
def Estimates_AH(polled_data, train_data, t_nodes, m, nodevec, tau, h_beta):
    ...
    lambda_t_all = np.maximum(B_S(m, T_O_all, nodevec) @ coefs[d:(d+m+4)], 0)
    g0_T_X_n = np.log(np.maximum(lambda_t_all + Beta_X_all, 1e-4)) 

    # -----------------------------------------
    lambda_t_nodes = np.maximum(B_S(m, t_nodes, nodevec) @ coefs[d:(d+m+4)], 0)
    Beta_X_train = X_train @ coefs[:d] 
    lambda_t_train = np.maximum(B_S(m, T_O_train, nodevec) @ coefs[d:(d+m+4)], 0)
