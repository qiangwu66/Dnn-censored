...
#%% ------------------------
def g_dnn_agq(X_R_train, De_R_train, Time_R_train, X_R_valid, De_R_valid, Time_R_valid, X_R_test, Time_R_test, s_k, n_layer, n_node, n_lr, n_epoch, patiences,
         agq_n_low, agq_n_high, agq_tol, agq_max_subdiv, clamp_exp_max=None):
    print('DNN_iteration')

    # ---------- Data tensors ----------
    X_train = torch.tensor(X_R_train, dtype=torch.float32)
    De_train = torch.tensor(De_R_train, dtype=torch.float32)
    T_O_train = torch.tensor(Time_R_train, dtype=torch.float32)

    X_val = torch.tensor(X_R_valid, dtype=torch.float32)
    De_val = torch.tensor(De_R_valid, dtype=torch.float32)
    T_O_val = torch.tensor(Time_R_valid, dtype=torch.float32)

    X_test = torch.tensor(X_R_test, dtype=torch.float32)
    T_O_test = torch.tensor(Time_R_test, dtype=torch.float32)
    ...
    model.eval()
    with torch.no_grad():
        # # fig: 在 t_fig 上的生存函数 S(t|X) = exp(-∫_0^t exp(g(s|X)) ds)
