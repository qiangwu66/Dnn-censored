...
from Survival_methods.AFT_iteration import Estimates_AFT
import pandas as pd
import os
...
# -------training data1: 45%  validation data1: 10%  training data2: 45%-----------------------
# ---training data1: 857
X_R_train1 = X_R[np.arange(857)]
De_R_train1 = De_R[np.arange(857)]
Time_R_train1 = Time_R[np.arange(857)]
# ---training data2: 857
X_R_train2 = X_R[np.arange(857, 1714)]
De_R_train2 = De_R[np.arange(857, 1714)]
Time_R_train2 = Time_R[np.arange(857, 1714)]
# ---validation data1: 190
X_R_valid1 = X_R[np.arange(1714, 1904)]
De_R_valid1 = De_R[np.arange(1714, 1904)]
Time_R_valid1 = Time_R[np.arange(1714, 1904)]
...
# ----------------AFT--------------
Est_AFT_g0 = Estimates_AFT(polled_data,train_data2, t_nodes, m, nodevec, tau)

sigma_1_n2_AFT = Est_AFT_g0['sigma_1_n2_AFT']
g0_T_X_n = Est_AFT_g0['g0_T_X_n']


#%%-----------Test Statistics-------------
sigma_n1 = np.sqrt(2 * (sigma_1_n1 ** 2 + sigma_1_n2_AFT ** 2))
I_T_T_n = Indicator_matrix(polled_data['T_O'],polled_data['T_O'])
I_T_T_mean = np.mean(I_T_T_n, axis=0) 
T_w_n = np.sqrt(1714) * np.mean(polled_data['De'] * I_T_T_mean * (g1_T_X_n - g0_T_X_n))
...
size_DNN_path = os.path.join(output_folder, f"AFTlinear_p_value.csv")
