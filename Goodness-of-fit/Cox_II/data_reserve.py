...
def generate_Cox_2(n, corr, tau):
    ...
    f_X = Z1 + 2 *Z2 * np.sin(X1[:,0] * np.pi) + np.log(X1[:,1] * X1[:,2] + np.exp(X1[:,3]) + 3) 
    Y = ndm.rand(n)
...
def save_data_for_r(n_values, B=500, corr=0.5, tau=2, results_folder="."):
    for n in n_values:
        ...
        for b in range(B):
            np.random.seed(490 + b) 
            

            data = generate_Cox_2(n, corr, tau)
        ...
        csv_filename = f"generated_data_n{n}_Cox2.csv"
