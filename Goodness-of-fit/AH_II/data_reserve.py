...
def generate_AH_2(n, corr, tau):
    ...
    f_X = Z1 / 2 + Z2 / 2 + np.cos(np.pi * (X1[:,0] + X1[:,1])) + np.sin(np.pi * (X1[:,2] + X1[:,3]) / 2)
    a = 2
    Y = ndm.rand(n)
...
def save_data_for_r(n_values, B=500, corr=0.5, tau=2, results_folder="."):
    for n in n_values:
        ...
        csv_filename = f"generated_data_n{n}_AH2.csv"
